...
// 입력된 정보가 다 안들어 왔다면 에러 메시지를 보내준다. 
    if (!req.body.location || !req.username || !req.body.password)
        ...
        try {
            ...
            //회원가입 하고 바로 로그인이 되도록 해주기 위해
            // session에 userID를 심어준다.  == 즉 그것이 id이다.
            req.session.userID = user._id;

            //생성된 user정보를 보낸다. 
            // (password는 userSchema 생성 때 작성한 toJSON가 실행되면서 사라져서 username과 location만 전달된다.)
            return res.send({
                user: user,
            });
        } catch (error) {
